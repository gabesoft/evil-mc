...
defining a variable named ~evil-mc-keys~ with the key bindings of your liking. 
...
- Most evil motions and operators are supported but not every command will work. 
...
**** Debugging
- When a command does not work you can enable or disable 
  debugging by running any of the commands below interactively.
  #+BEGIN_SRC emacs-lisp
...
**** Limitations
- After undo the cursors will return to their original positions
  if [[http://www.emacswiki.org/emacs/UndoTree][undo-tree]] mode is enabled and ~evil-repeat~ is not used.
